...
| AEMS | Operational engagement authorization, planning, AEMS-2A/2B Planning Package artifacts and workspace, AEMS-3A/3B team safeguards and ARMIS provider gate, AEMS-4A/4B fieldwork execution, AEMS-5A/5B Evidence Requests and immutable evidence assessments, Working Papers/Evidence, Issues, Findings, responses, conferences, interim/final reporting and distribution, CMS transfer, completion, closure, retention/index, controlled reopening, AEMS-10A/10B scope-aware dashboard, work queues, protected exports, notification monitoring, configurable reminder rules, and AEMS-11 cross-module ownership/security hardening are implemented as an operational subset. AIS integration remains outside the current scope; reference-conformance gaps remain tracked by the G0 contract. | As-built behavior in [AEMS Workflow Design](AEMS_WORKFLOW_DESIGN.md), [AEMS Cross-Module Integration](AEMS_CROSS_MODULE_INTEGRATION.md), [AEMS Implementation Baseline](AEMS_IMPLEMENTATION_BASELINE.md), and the [AEMS G0 Governance and Conformance Contract](AEMS_G0_GOVERNANCE_CONFORMANCE_CONTRACT.md), with API, testing, and system-flow references |
...
9. [AEMS Cross-Module Integration](AEMS_CROSS_MODULE_INTEGRATION.md)
10. [CMS Workflow Design](CMS_WORKFLOW_DESIGN.md)
11. [ARMIS Workflow and Implementation Checkpoint](ARMIS_WORKFLOW_DESIGN.md)
12. [API and Data Reference](API_AND_DATA_REFERENCE.md)
13. [Operations Guide](OPERATIONS_GUIDE.md)
14. [End-to-End Testing Guide](END_TO_END_TESTING_GUIDE.md)
