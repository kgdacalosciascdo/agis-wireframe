...
function resolverFor(blocker) {
  ...
  if (["RESOURCE_PROVIDER_UNAVAILABLE", "RESOURCE_PROVIDER_STALE", "RESOURCE_AUTHORITY_NOT_ELIGIBLE"].includes(blocker.code)) {
    return "ARMIS/resource administrator must refresh or accept the provider reconciliation.";
  }
...
export default function AemsTeamSafeguardsPanel({ engagementId, overview, loading, onRefresh }) {
  ...
  async function saveDeclaration() {
    ...
    try {
      ...
      toast.success("Safeguard declaration submitted for independent review.");
  ...
  return (
    <section className="mt-5 space-y-5" data-testid="aems-team-safeguards-panel">
      ...
      <section className="rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="grid gap-4 p-4 sm:grid-cols-2 sm:p-5">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="mt-2 text-xs leading-5 text-slate-600">{overview.provider?.authoritative ? "ARMIS is authoritative for assignment capacity and person-days." : "IAP is the explicit interim fallback; ARMIS data is not authoritative."}</p>
            <p className="mt-2 text-xs text-slate-500">Last accepted reconciliation: {dateTime(overview.provider?.latestReconciliation?.generatedAt)} · freshness window {overview.provider?.snapshotMaxAgeDays ?? 30} days</p>
          </div>
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
            <p className="text-xs font-bold uppercase tracking-wide text-slate-500">Reconciliation</p>
            ...
            <div className="mt-3 flex items-center gap-2 text-xs">{overview.reconciliation?.planned?.reconciled ? <CheckCircle2 size={15} className="text-emerald-600" /> : <AlertTriangle size={15} className="text-red-600" />}<span className="font-semibold text-slate-700">Planned effort {overview.reconciliation?.planned?.reconciled ? "reconciles" : "does not reconcile"}.</span></div>
            <p className="mt-2 text-xs text-slate-500">Actuals: team {number(overview.reconciliation?.actual?.team)} · provider {number(overview.reconciliation?.actual?.provider)} · engagement {number(overview.reconciliation?.actual?.engagement)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <table className="min-w-[980px] w-full text-left text-xs"><thead className="bg-slate-50 text-[11px] uppercase tracking-wide text-slate-500"><tr><th className="px-4 py-3">Member / role</th><th className="px-4 py-3">Competencies & certifications</th><th className="px-4 py-3">Planned / actual</th><th className="px-4 py-3">Capacity</th><th className="px-4 py-3">Availability & conflicts</th><th className="px-4 py-3">Declarations</th></tr></thead><tbody className="divide-y divide-slate-100">{(overview.team ?? []).map((member) => { const memberDeclarations = declarations.filter((item) => item.engagementTeamId === member.teamMemberId); return <tr key={member.teamMemberId} className="align-top"><td className="px-4 py-4"><strong className="block text-sm text-slate-800">{member.user?.name ?? "Unknown"}</strong><span className="text-slate-500">{member.user?.employeeId} · {label(member.role)}</span></td><td className="max-w-64 px-4 py-4"><div className="flex flex-wrap gap-1">{(member.competencies ?? []).map((skill) => <span className="rounded-full bg-sky-50 px-2 py-1 text-[11px] text-sky-700" key={`${member.teamMemberId}-${skill.id ?? skill.code}`}>{skill.label ?? skill.name} {skill.proficiencyLevel ? `· ${skill.proficiencyLevel}` : ""}</span>)}{!(member.competencies ?? []).length && <span className="text-red-600">No competency record</span>}</div>{(member.certifications ?? []).length > 0 && <p className="mt-2 text-[11px] text-emerald-700">Certifications: {member.certifications.map((item) => item.label ?? item.name).join(", ")}</p>}</td><td className="whitespace-nowrap px-4 py-4"><strong>{number(member.plannedPersonDays)} days</strong><span className="block text-slate-500">{number(member.actualPersonDays)} actual</span></td><td className="whitespace-nowrap px-4 py-4"><StatusBadge tone={member.capacity?.met ? "success" : "danger"}>{member.capacity?.met ? "Within capacity" : "Over capacity"}</StatusBadge><span className="mt-1 block text-slate-500">{number(member.capacity?.allocated)} / {number(member.capacity?.available)} days</span></td><td className="max-w-56 px-4 py-4">{member.overlap && <p className="font-semibold text-red-700">Overlap: {member.overlap.engagementCode}</p>}{(member.unavailability ?? []).map((item) => <p className="mt-1 text-red-700" key={`${item.startDate}-${item.endDate}`}>{item.typeLabel ?? item.title}: {item.startDate}–{item.endDate}</p>)}{!member.overlap && !(member.unavailability ?? []).length && <span className="text-emerald-700">No conflicts reported</span>}</td><td className="min-w-56 px-4 py-4"><div className="space-y-1">{declarationTypes.map(([type, typeLabel]) => { const declaration = memberDeclarations.find((item) => item.declarationType === type); return <div className="flex items-center justify-between gap-2" key={type}><span className="text-slate-600">{typeLabel}</span>{declaration ? <button className="text-left" onClick={() => declaration.status === "SUBMITTED" && canReview ? setReviewTarget({ member, declaration }) : canDeclare && ["ACCEPTED", "RETURNED"].includes(declaration.status) ? openDeclaration(member, declaration) : undefined} type="button"><StatusBadge tone={toneForStatus(declaration.status)}>{label(declaration.status)} · v{declaration.versionNumber}</StatusBadge></button> : <StatusBadge tone="danger">Missing</StatusBadge>}</div>; })}</div><div className="mt-3 flex flex-wrap gap-2">{canDeclare && <button className="inline-flex items-center gap-1 rounded-lg border border-sky-200 px-2.5 py-1.5 text-[11px] font-bold text-sky-700 hover:bg-sky-50" onClick={() => openDeclaration(member)} type="button"><UserCheck size={13} /> Submit</button>}{canReview && memberDeclarations.some((item) => item.status === "SUBMITTED") && <span className="text-[11px] font-semibold text-amber-700">Review required</span>}</div></td></tr>; })}</tbody></table>
      ...
      <Modal open={Boolean(declarationTarget)} onClose={() => !saving && setDeclarationTarget(null)} size="lg" title={`${declarationTarget?.existing ? "Revise" : "Submit"} safeguard declaration`} description="The declaration is versioned and must be accepted by an independent reviewer before assignment approval.">
      ...
      <Modal open={Boolean(reviewTarget)} onClose={() => !saving && setReviewTarget(null)} size="md" title="Review safeguard declaration" description="Review independently. Returning a declaration requires a clear explanation for the assigned resource.">
