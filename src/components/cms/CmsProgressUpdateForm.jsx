...
function MilestoneCard({ definition, entry, index, errors, onChange, evidenceCount }) {
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        >
          <select
            ...
            onChange={(event) => set("managementReportedStatusCode", event.target.value)}
        ...
        >
          <input
            ...
            max="100"
            min="0"
            onChange={(event) => set("managementReportedPercentage", event.target.value)}
            step="0.01"
            type="number"
        ...
        <div className="md:col-span-2">
          ...
          >
            <textarea
              ...
              maxLength={10000}
              onChange={(event) => set("accomplishmentDescription", event.target.value)}
        ...
        >
          <textarea
            ...
            maxLength={10000}
            onChange={(event) => set("issuesAndConstraints", event.target.value)}
        ...
        >
          <input
            ...
            onChange={(event) => set("forecastCompletionDate", event.target.value)}
            type="date"
        ...
        <div className="md:col-span-2">
          ...
          >
            <textarea
              ...
              maxLength={10000}
              onChange={(event) => set("noEvidenceExplanation", event.target.value)}
...
export default function CmsProgressUpdateForm({
  ...
  const summaryErrors = Object.entries(errors ?? {})
    ...
    .flatMap(([key, values]) =>
      (Array.isArray(values) ? values : [values]).map((message) => ({ key, message })),
    );
    ...
    >
      {summaryErrors.length > 0 && (
        <section
          ...
          role="alert"
        >
          <h3 className="text-sm font-bold text-red-800" id="progress-validation-summary">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <h3 className="text-base font-bold text-slate-800">Management progress report</h3>
            <p className="mt-1 text-xs leading-5 text-slate-500">
              This workspace records management-reported progress and supporting evidence. It does not independently validate implementation.
            </p>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          >
            <input
              ...
              onChange={(event) => set("reportingPeriodStart", event.target.value)}
              type="date"
          ...
          >
            <input
              ...
              onChange={(event) => set("reportingPeriodEnd", event.target.value)}
              type="date"
          ...
          <div className="md:col-span-2">
            ...
            >
              <textarea
                ...
                maxLength={10000}
                onChange={(event) => set("accomplishmentSummary", event.target.value)}
          ...
          <FormField
            ...
            hint={form.baselineWeighted ? "Calculated by the server from milestone weights." : "Enter management's overall reported percentage. The system does not average unweighted milestones."}
          >
            <input
              className={inputClass}
              ...
              max="100"
              min="0"
              onChange={(event) => set("managementReportedOverallPercentage", event.target.value)}
              step="0.01"
              type="number"
          ...
          <div className="rounded-lg border border-sky-200 bg-sky-50 p-3 text-sm text-sky-900">
            <strong>{form.baselineWeighted ? "Weighted baseline" : "Unweighted baseline"}</strong>
            <p className="mt-1 text-xs leading-5">
              {form.systemCalculatedWeightedReportedPercentage !== null && form.systemCalculatedWeightedReportedPercentage !== undefined
          ...
          <div className="md:col-span-2">
            <FormField
              ...
              label="Issues and constraints"
            >
              <textarea className={textareaClass} id="progress-issues" maxLength={10000} onChange={(event) => set("issuesAndConstraints", event.target.value)} value={form.issuesAndConstraints ?? ""} />
            </FormField>
          </div>
          <FormField error={firstError(errors, "correctiveActionsForDelays")} htmlFor="progress-delays" label="Corrective actions for delays">
            <textarea className={textareaClass} id="progress-delays" maxLength={10000} onChange={(event) => set("correctiveActionsForDelays", event.target.value)} value={form.correctiveActionsForDelays ?? ""} />
          </FormField>
          <FormField error={firstError(errors, "nextSteps")} htmlFor="progress-next-steps" label="Next steps">
            <textarea className={textareaClass} id="progress-next-steps" maxLength={10000} onChange={(event) => set("nextSteps", event.target.value)} value={form.nextSteps ?? ""} />
          </FormField>
          <FormField error={firstError(errors, "forecastCompletionDate")} htmlFor="progress-forecast" label="Forecast completion date">
            <input className={inputClass} id="progress-forecast" onChange={(event) => set("forecastCompletionDate", event.target.value)} type="date" value={form.forecastCompletionDate ?? ""} />
          </FormField>
          <FormField error={firstError(errors, "managementDeclaration")} htmlFor="progress-declaration" label="Management declaration">
            <textarea className={textareaClass} id="progress-declaration" maxLength={10000} onChange={(event) => set("managementDeclaration", event.target.value)} value={form.managementDeclaration ?? ""} />
          </FormField>
          <div className="md:col-span-2">
            <FormField error={firstError(errors, "generalEvidenceExplanation")} htmlFor="progress-general-evidence" label="General evidence explanation" hint="A supporting document or suitable explanation is required before submission.">
              <textarea className={textareaClass} id="progress-general-evidence" maxLength={10000} onChange={(event) => set("generalEvidenceExplanation", event.target.value)} value={form.generalEvidenceExplanation ?? ""} />
            </FormField>
          </div>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-slate-200 bg-slate-50 p-4 sm:p-5">
        <div>
          <h3 className="text-base font-bold text-slate-800">Milestone progress</h3>
          <p className="mt-1 text-xs leading-5 text-slate-500">
            The accepted Action Plan baseline is fixed. Every accepted milestone must be reported exactly once, and its wording, owner, dates, and weight cannot be changed here.
          </p>
        </div>
        <div className="mt-4 grid gap-4">
          {definitions.map((definition, index) => (
            <MilestoneCard
              definition={milestoneDefinition(definition, entries[index], index)}
              entry={entries[index] ?? { actionPlanMilestoneId: definition.id, managementReportedStatusCode: "NOT_STARTED", managementReportedPercentage: 0, displayOrder: index + 1 }}
        ...
        {definitions.length === 0 && (
          <p className="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">
            No accepted Action Plan milestones are available. Progress reporting cannot be submitted until an accepted baseline is available.
          </p>
        )}
      </section>
      ...
      <div className="sticky bottom-3 z-20 flex flex-wrap justify-end gap-2 rounded-xl border border-slate-200 bg-white/95 p-3 shadow-lg backdrop-blur">
        {isCreate && (
          <button className="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-60" disabled={busy} onClick={onCancel} type="button">
            <X size={16} /> Cancel
          </button>
        )}
        <button className="inline-flex h-10 items-center gap-2 rounded-lg bg-sky-700 px-5 text-sm font-bold text-white hover:bg-sky-800 disabled:cursor-not-allowed disabled:opacity-60" disabled={busy} type="submit">
          <Save size={16} /> {busy ? "Saving..." : isCreate ? "Create draft" : "Save draft"}
        </button>
      </div>
    </form>
  );
